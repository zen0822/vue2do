...
BuildCSS4WebpackPlugin.build = function ({
  ...
  const travel = function travel(travelDir, callback) {
    fs.readdirSync(travelDir).forEach(function (file) {
      const pathname = path.join(travelDir, file)
