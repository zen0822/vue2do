...
<template>
    <div @click.stop=""  class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
        ...
        <div class="calendar-time" v-show="type=='datetime'||type=='time' || type == 'datetimeS'">
            ...
            <div class="timer">
                ...
                分
                <template v-if="type != 'datetimeS'">
                <input type="text" v-model="second" value="{{second}}" min="0" max="59" maxlength="2">
                秒</template>
            </div>
        </div>
        <div class="calendar-button" v-show="type=='datetime'||type=='time'||range || type=='datetimeS'">
    ...
    methods: {
        ...
        init(){
            ...
            if (this.value != "") {
                ...
                if (this.type == "date") {
                ...
                } else if (this.type == "datetime" || this.type=='datetimeS') {
                    ...
                    if (split.length > 1) {
                        ...
                        if(this.type!='datetimes'){this.second = splitTime[2]}
                    }
                }
                if (this.range) {
                    var split = this.value.split(" ~ ")
        ...
        output(args) {
            ...
            if (this.type == 'datetimeS') {
                return args[0] + this.sep + this.zero(args[1] + 1) + this.sep + this.zero(args[2]) + " " + this.zero(args[3]) + ":" + this.zero(args[4])
            }
            if (this.type == 'date') {
